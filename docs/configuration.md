...
FM_GUARD_NO_ATTENTION=0 # suppress fm-guard.sh's captain's-call section for a caller that prints it itself (docs/captain-attention.md)
...
FM_ATTENTION_TURNEND_BLOCK=1   # set to 0 to disable the turn-end captain-decision stop without touching the watcher-liveness backstop (docs/captain-attention.md)
