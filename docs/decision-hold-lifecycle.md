...
The same subcommand records the durable captain briefing that makes the decision answerable: the concrete choice, why it matters now, what waiting costs, repeatable options, and a recommendation, each one line and free of backslashes so the stored body round-trips unambiguously.
...
Supplying any briefing flag rewrites the block and preserves unrecognized body lines; supplying none leaves an existing briefing untouched, so an idempotent retry cannot erase one.
