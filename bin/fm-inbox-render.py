...
def decision_view(record, task, full_text, cards):
    ...
    options = card.get("options") or [
        "Go with firstmate's recommendation",
        "Choose differently (say how in the note)",
...
/* Long ids, URLs and hold text must wrap rather than widen the page at any
   nesting level; wide blocks scroll inside their own container instead. */
a{color:var(--accent); overflow-wrap:anywhere}
.card,.field,.field p,.flag,form.answer{min-width:0; overflow-wrap:anywhere}
...
form.answer .send{display:flex; align-items:center; gap:.75rem; margin-top:.75rem; flex-wrap:wrap}
form.answer button{
...
form.answer .queued{font-size:.84rem; color:var(--ok)}
...
function fmInboxQueue(form){
  ...
  var text='DECISION '+id+': '+answer+(note?' -- '+note:'');
  var mark=form.querySelector('.queued');
  if(window.lavish&&window.lavish.queuePrompt){
    window.lavish.queuePrompt(text,{
      ...
      data:{hold:id,question:question,answer:answer,note:note}
    });
    if(mark){mark.textContent='Queued. Press Send to Agent when you are done.';}
  }else if(mark){
    mark.textContent='Open this board through Lavish to send answers.';
  }
}
"""
...
def render_decision(view):
    ...
    for index, option in enumerate(view["options"]):
        opts.append(
            '<label class="opt"><input type="radio" name="answer" value="%s"%s>'
            "<span>%s</span></label>"
            % (esc(option), "" if index else "", esc(option))
        )
    parts.append(
        '<form class="answer" data-lavish-question="%s" data-question="%s" '
        'onsubmit="event.preventDefault();fmInboxQueue(event.currentTarget);">'
        '<p class="label">Your answer</p>%s'
        '<textarea name="note" placeholder="Anything to add (optional)"></textarea>'
        '<div class="send"><button type="submit">Queue this answer</button>'
        '<span class="queued"></span></div></form>'
        % (esc(view["id"]), esc(view["question"]), "".join(opts))
    )
...
def render(model, snap, home, pr_verified):
    ...
    for record, task, _ in model["stuck"]:
        projects.add(project_of(record, task))
    for record, task in model["underway"]:
        projects.add(project_of(record, task))
    for record in model["queued"] + model["shipped"]:
        projects.add(project_of(record, model["tasks"].get(record.get("id"))))
