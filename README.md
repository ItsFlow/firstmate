...
- **Nothing waits silently** - every open decision and meaningful delay is collected in one captain-facing place, in plain English with the concrete choice, what waiting costs, and a recommendation; each one is surfaced once, stays listed until it is answered or clears, and a home holding only an unanswered decision is no longer treated as idle.
